...
## Последние продуктовые решения

1. На главной больше нет карточки `Сегодняшний выбор`: экран короче и ведёт сразу в три основных раздела.
2. Home и `Помоги приготовить` оформлены как более контрастные "разделочные доски": карточки темнее, насыщеннее и лучше отделены от фона.
3. Если холодильник пуст, экран `Помоги приготовить` не показывает `Лучшее блюдо сегодня` и не делает вид, что уже знает, что готовить. Вместо этого показывается пустое состояние с переходом в холодильник.
4. Если подходящего шаблона в базе нет, встроенный шеф может собрать локальную `Шеф-идею`, но только после проверки валидаторами и anti-duplicate логикой.
5. Пользовательские причины на карточках рецептов должны звучать по-человечески. Внутренние технические формулировки вроде `защита от пересушивания` в UI не допускаются.
...
Android release-сборка по ABI:

```powershell
./flutter/bin/flutter.bat build apk --release --split-per-abi
```

Практическое правило для APK:

- `app-arm64-v8a-release.apk` — основной APK для современных Android-телефонов
- `app-x86_64-release.apk` — APK для Android-эмулятора
- `app-armeabi-v7a-release.apk` — APK для старых 32-bit устройств
